...
<style>
  ...
  h2 { margin: 0 0 16px 0; font-size: 18px; color: #16213e; border-bottom: 2px solid #0f3460; padding-bottom: 8px; }
  ...
  .big { font-size: 20px; font-weight: 700; color: #0f3460; }
</style>
"""
...
def notify_build_completed(
    ...
    html_parts = [_kv_table([
        ("Build type", f"<b>{build_type}</b>"),
        ("Trigger", trigger),
        ("Duration", f"<span class='big'>{_fmt_duration(duration_sec)}</span>" if duration_sec else "unknown"),
    ...
    if commit_messages:
        ...
        more = f"<div style='color:#8b8fa3;font-size:12px'>… and {len(commit_messages) - 20} more</div>" if len(commit_messages) > 20 else ""
    ...
    if changed_files:
        ...
        total_row = f"<tr><td><b>Total ({len(changed_files)} files)</b></td><td style='text-align:right'><b>{len(changed_files)}</b></td></tr>"
        html_parts.append(
            f"<div class='section'>Files changed — by area</div>"
            f"<table><tr><th>Directory</th><th style='text-align:right'>Files</th></tr>{rows}{total_row}</table>"
        )
    ...
    if high_impact_symbols:
        ...
        html_parts.append(
            f"<div class='section'>High-impact symbols changed (many dependents)</div>"
    ...
    if symbols_added is not None or symbols_removed is not None:
        delta_row = f"<tr><td><b>Symbol delta</b></td><td><span class='ok'>+{symbols_added or 0}</span> / <span class='fail'>-{symbols_removed or 0}</span></td></tr>"
    html_parts.append(
        f"<div class='section'>Graph stats</div>"
        f"<table>"
        f"<tr><td><b>Graph nodes</b></td><td>{graph_nodes:,}</td></tr>"
        f"<tr><td><b>Graph edges</b></td><td>{graph_edges:,}</td></tr>"
        f"<tr><td><b>Semantic documents</b></td><td>{semantic_documents:,}</td></tr>"
        f"{delta_row}"
        f"</table>"
    )
